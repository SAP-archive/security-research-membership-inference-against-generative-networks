...
def reconstruction_attack(trX_inds, vaX_inds, repeat, n):
    ...
    for i in range(len(vaX_inds)):
        ...
        print_elapsed_time()
        results_sample[i][1] = compute_avg_rec_error(vaX[vaX_inds][i], vaY[vaX_inds][i], repeat, n)
    ...
    for i in range(len(trX_inds)):
        ...
        print_elapsed_time()
        results_train[i][1] = compute_avg_rec_error(trX[trX_inds][i], trY[trX_inds][i], repeat, n)
    ...
    new_row['mc_euclidean_no_batches'] = repeat*n
...
n_s = [1, 2, 1, 2, 1, 2, 1, 4, 10]
repeats = [300, 500, 3000, 5000, 30000, 50000, 300000, 250000, 300000]
...
for exp_no in range(exp_nos):
    for i in range(len(n_s)):
        ...
        reconstruction_attack(trX_inds, vaX_inds, repeat, n) # production ready
