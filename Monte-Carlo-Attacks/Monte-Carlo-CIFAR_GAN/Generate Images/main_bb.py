...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='cifar10', choices=['mnist', 'fashion-mnist', 'celebA','cifar10'], #add cifar10
    ...
    parser.add_argument('--directory', type=str, default='./train',
                        help='Directory for cifar-10 images')
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        ...
        if args.gan_type == 'GAN':
            gan = GAN(sess, epoch=args.epoch, batch_size=args.batch_size, dataset_name=args.dataset,
                      checkpoint_dir=args.checkpoint_dir, result_dir=args.result_dir, log_dir=args.log_dir, directory=args.directory, reuse=True, black_box=True)
            '''
