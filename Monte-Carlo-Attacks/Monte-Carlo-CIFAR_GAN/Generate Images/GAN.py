...
class GAN(object):
    def __init__(self, sess, epoch, batch_size, dataset_name, checkpoint_dir, result_dir, log_dir, directory, reuse=False, black_box=False):
        self.sess = sess
        ...
        if dataset_name == 'mnist' or dataset_name == 'fashion-mnist': # fix
            ...
            # load cifar10
            if black_box:
                self.data_X = np.load('sample_matrix.npy')
            else:
                self.data_X, self.data_y,_,_ = load_cifar10_with_validation('./train', reuse=reuse)
    ...
    def generator(self, z, is_training=True, reuse=False, alt_batch_size=100):
        # Network Architecture is exactly same as in infoGAN (https://arxiv.org/abs/1606.03657)
        ...
        with tf.variable_scope("generator", reuse=reuse):
            ...
            if self.dataset_name == 'cifar10':
                ...
                net = tf.nn.relu(
                    bn(tf.reshape(net, [alt_batch_size, h_size_16, h_size_16, 512]),is_training=is_training, scope='g_bn1')
                    )
                print("G:",net.get_shape())
                net = tf.nn.relu(
                    bn(deconv2d(net, [alt_batch_size, h_size_8, h_size_8, 256], 5, 5, 2, 2, name='g_dc2'+'_'+self.dataset_name),is_training=is_training, scope='g_bn2')
                    )
                print("G:",net.get_shape())
                net = tf.nn.relu(
                    bn(deconv2d(net, [alt_batch_size, h_size_4, h_size_4, 128], 5, 5, 2, 2, name='g_dc3'+'_'+self.dataset_name),is_training=is_training, scope='g_bn3')
                    )
                print("G:",net.get_shape())
                net = tf.nn.relu(
                    bn(deconv2d(net, [alt_batch_size, h_size_2, h_size_2, 64], 5, 5, 2, 2, name='g_dc4'+'_'+self.dataset_name),is_training=is_training, scope='g_bn4')
                    )
                print("G:",net.get_shape())
                out = tf.nn.tanh(
                    deconv2d(net, [alt_batch_size, self.output_height, self.output_width, self.c_dim], 5, 5, 2, 2, name='g_dc5'+'_'+self.dataset_name)
                    )
                ...
                net = tf.reshape(net, [alt_batch_size, h_size_4, h_size_4, 128]) #  8 8  128
                net = tf.nn.relu(
                    bn(deconv2d(net, [alt_batch_size, h_size_2, h_size_2, 64], 4, 4, 2, 2, name='g_dc3'+'_'+self.dataset_name), is_training=is_training,scope='g_bn3')
                    )

                out = tf.nn.sigmoid(deconv2d(net, [alt_batch_size, self.output_height, self.output_width, self.c_dim], 4, 4, 2, 2, name='g_dc4'+'_'+self.dataset_name))
    ...
    def build_model(self):
        ...
        """" Testing """
        # for test
        self.bs_sample = 10000
        self.z_sample = tf.placeholder(tf.float32, [self.bs_sample, self.z_dim], name='z_sample')
        self.fake_images_sample = self.generator(self.z_sample, is_training=False, reuse=True, alt_batch_size=self.bs_sample)
    ...
    def sample(self):
        z_sample = np.random.uniform(-1, 1, size=(self.bs_sample, self.z_dim)) # 100, 100
        samples = self.sess.run(self.fake_images_sample, feed_dict={self.z_sample: z_sample})
        return samples
